...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    // Count Sketch Observation Window Annotation (for resetting the frequency values to 0 in the new observation window)
    ...
    // (Moving average and moving difference)
    ...
    register<bit<8>>(1) alpha;    // Fixed point representation: 0 integer bits, 8 fractional bits. Set to 0.125    
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            if (meta.pkt_num != m) {
                ...
                if (current_ow == 1) {
                    // Initialize moving average and difference in the first observation window.
                    meta.src_ewma = meta.src_entropy << 14;
                    meta.src_ewmmd = 0;
                    meta.dst_ewma = meta.dst_entropy << 14;
                    meta.dst_ewmmd = 0;
                } else {
                    // If alarm is 1, then ddos attack has been detected.
                    meta.alarm = 0;
                ...
                bit<32> session;
                random(session, 0, 3);
                clone3(CloneType.I2E, session, { meta.pkt_num, meta.src_entropy, meta.src_ewma, meta.src_ewmmd, meta.dst_entropy, meta.dst_ewma, meta.dst_ewmmd, meta.alarm });
